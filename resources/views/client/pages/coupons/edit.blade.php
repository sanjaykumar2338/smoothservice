...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">Marketing /</span> Coupons / Edit Coupon
    </h4>
    ...
    <div class="row">
        <div class="col-xl">
            <form id="coupon_form" method="POST" action="{{ route('coupon.update', $coupon->id) }}">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" id="coupon_code" name="coupon_code" value="{{ old('coupon_code', $coupon->coupon_code) }}" placeholder="E.G. 25OFF" />
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" placeholder="Optional">{{ old('description', $coupon->description) }}</textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex">
                                <div class="form-check me-3">
                                    <input class="form-check-input" type="radio" name="discount_type" id="fixed_amount" value="Fixed" {{ $coupon->discount_type == 'Fixed' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="fixed_amount">
                                        Fixed Amount
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discount_type" id="percentage" value="Percentage" {{ $coupon->discount_type == 'Percentage' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex">
                                <div class="form-check me-3">
                                    <input class="form-check-input" type="radio" name="discount_duration" id="forever" value="Forever" {{ $coupon->discount_duration == 'Forever' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="forever">
                                        Forever
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="discount_duration" id="first_payment" value="First Payment" {{ $coupon->discount_duration == 'First Payment' ? 'checked' : '' }}>
                        ...
                        <div class="discount-wrapper">
                            @foreach($couponServices as $index => $couponService)
                                <div class="mb-3 discount-clone d-flex align-items-center">
                                    <!-- Applies to (Dropdown) -->
                                    <div class="flex-grow-1 me-3">
                                        <label class="form-label" for="applies_to_{{ $index }}">Applies to</label>
                                        <select class="form-control applies_to" id="applies_to_{{ $index }}" name="applies_to[{{ $index }}][]">
                                            @foreach($services as $serviceOption)
                                                <option value="{{ $serviceOption->id }}" {{ in_array($serviceOption->id, (array) $couponService['service_id']) ? 'selected' : '' }}>
                                    ...
                                    <!-- Discount Input -->
                                    <div class="flex-grow-1 me-3">
                                        <label class="form-label" for="discount_{{ $index }}">Discount</label>
                                        <input type="text" class="form-control" id="discount_{{ $index }}" name="discount[{{ $index }}]" value="{{ old('discount.' . $index, $couponService['discount']) }}" placeholder="e.g. $10">
                                    </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="limit_to_one" id="limit_to_one" value="1" {{ $coupon->limit_to_one ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="limit_to_new_customers" id="limit_to_new_customers" value="1" {{ $coupon->limit_to_new_customers ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="set_expiry" id="set_expiry" value="1" {{ $coupon->expiry_date ? 'checked' : '' }}>
                                <label class="form-check-label" for="set_expiry">Set expiry date</label>
                            </div>
                            <!-- Expiry date input -->
                            <div class="mt-2" id="expiry_date_field" style="{{ $coupon->expiry_date ? 'block' : 'none' }}">
                                <label for="expiry_date">Expiry Date</label>
                                <input type="date" id="expiry_date" name="expiry_date" class="form-control" value="{{ old('expiry_date', $coupon->expiry_date) }}">
                            </div>

                            <!-- Require a minimum cart amount (checkbox and input for price) -->
                            <div class="form-check form-switch mt-2">
                                <input class="form-check-input" type="checkbox" name="min_cart_amount" id="min_cart_amount" value="1" {{ $coupon->min_cart_amount ? 'checked' : '' }}>
                                <label class="form-check-label" for="min_cart_amount">Require a minimum cart amount</label>
                            </div>
                            <!-- Minimum cart amount input -->
                            <div class="mt-2" id="min_cart_amount_field" style="{{ $coupon->min_cart_amount ? 'block' : 'none' }}">
                                <label for="min_cart_amount_value">Minimum Cart Amount</label>
                                <input type="number" id="min_cart_amount_value" name="min_cart_amount_value" class="form-control" value="{{ old('min_cart_amount_value', $coupon->min_cart_amount) }}" placeholder="Enter minimum cart amount">
                            </div>
                        </div>
