...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">Subscriptions /</span> Edit 
    </h4>
    ...
    <div class="row">
        <div class="col-xl">
            ...
            <form id="subscription_form" method="POST" action="{{ route('subscriptions.update', $subscription->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-8">
                            ...
                            <select class="form-control" id="client" name="client_id">
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ $subscription->client_id == $client->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control" id="due_date" name="due_date" value="{{ old('due_date',$subscription->due_date) }}">
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea class="form-control" name="note" placeholder="Add a note for the client">{{ $subscription->note }}</textarea>
                        </div>

                        <!-- Additional Options with Dynamic Fields -->
                        <div class="mt-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input toggle-field" type="checkbox" id="send_email" name="send_email" {{ $subscription->send_email ? 'checked' : '' }}>
                        ...
                        <div class="mt-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input toggle-field" type="checkbox" id="partial_payment" name="partial_payment" data-toggle="upfront_payment_field" {{ $subscription->upfront_payment_amount ? 'checked' : '' }}>
                            ...
                            <div class="hidden-field" id="upfront_payment_field">
                                <input type="number" class="form-control mt-2" name="upfront_payment_amount" value="{{ $subscription->upfront_payment_amount }}" placeholder="Enter upfront payment amount">
                            </div>
                        </div>

                        <div class="mt-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input toggle-field" type="checkbox" id="custom_billing_date" name="custom_billing_date" data-toggle="billing_date_field" {{ $subscription->billing_date ? 'checked' : '' }}>
                            ...
                            <div class="hidden-field" id="billing_date_field">
                                <input type="date" class="form-control mt-2" name="billing_date" value="{{ $subscription->billing_date }}">
                            </div>
                        </div>

                        <div class="mt-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input toggle-field" type="checkbox" id="custom_currency" name="custom_currency" data-toggle="currency_field" {{ $subscription->currency ? 'checked' : '' }}>
                                <label class="form-check-label" for="custom_currency">Custom currency</label>
                            </div>
                            <div class="hidden-field" id="currency_field">
                                <input type="text" class="form-control mt-2" name="currency" value="{{ $subscription->currency }}" placeholder="Enter currency code (e.g., USD, CAD)">
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let itemIndex = {{ count($subscription->items) }};
